...
//        Sprite _DebugSprite;
    ...
    public override void OnLoad()
        ...
        {
            this.SpriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, BrainGame.CurrentSampler, null, null, null, SnailsGame.DefaultCamera.Transform);
          ...
          //          this._DebugSprite.Draw(this._Position + position, key._XboxControlFrameNr, this._spriteBatch);
                ...
                {
                    this.SpriteBatch.DrawString(this._Font, string.Format("({0})", key._Key.ToString()),
                    this._Position + position + new Vector2(0.0f, 15.0f), this._Textcolor, 0.0f, Vector2.Zero, 1.0f, SpriteEffects.None, 0);
                }
                this.SpriteBatch.DrawString(this._Font, key._HelpText,
        ...
        private void GenerateThumbsTexture(int theme)
        {
            ...
            for (int stageId = 1; stageId <= 21; stageId++)
            {
                ...
                if (ssDataFile != null)
                {
                    DataFileRecord stageRec = ssDataFile.RootRecord.SelectRecordByField("Sprite", "Id", Levels._instance.StagesByTheme[(int)theme][stageId - 1].StageKey);
                    if (stageRec != null)
                    {
