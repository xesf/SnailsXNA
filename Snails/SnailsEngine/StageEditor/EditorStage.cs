...
namespace LevelEditor
{
    public class EditorStage
    {
        ...
        */
        ...
        /// 
        /// </summary>
        public void SetTileAt(int styleGroupId, int x, int y)
        {
            if (this.Stage == null)
                return;
            this.Stage.Board.SetTileAt(styleGroupId, x, y);
            this.Changed = true;
        }
        ...
        public List<Object> MoveObjectsAndTiles(List<object> objectsAndTilesList, int offsetX, int offsetY, int offsetColRowX, int offsetColRowY)
        {
            ...
            foreach (Object obj in objectsAndTilesList)
            {
                ...
                else
                {
                    if (obj is TileCell)
                    {
                        ...
                        {
                            TileCell cell = (TileCell)obj;
                            this.Board.RemoveTileAt(cell.BoardX, cell.BoardY);
                            continue;
                        }
                    }
                }
            ...
            //   int offsetColRowX = (int)(offsetX / this.TileSize.Width);
      //   int offsetColRowY = (int)(offsetY / this.TileSize.Height);
            ...
            foreach (TileCell cell in tmpCells)
            {
                ...
                this.Board.Tiles[ry, rx] = new TileCell(this.Board, null, rx, ry);

                if (cell.Tile != null)
                {
                    this.Board.SetTileAt(cell.Tile.StyleGroupId, rx, ry);
                    objectsAndTilesMoved.Add(this.Board.Tiles[ry, rx]);
                }
            }
