        ...
        public override void Update(BrainEngine.BrainGameTime gameTime)
        {
            ...
            switch (this._state)
            {
                case PumpState.Working:
                    ...
                    this._wheelRotation += (WHEEL_ROTATION_SPEED * (float)gameTime.ElapsedGameTime.TotalMilliseconds / 1000f);
                    if (this.PumpType == PumpTypes.PumpIn)
                    {
                        ...
                        foreach (Liquid liquid in this.LinkedObjects)
                        {
                            if (!liquid.IsFull)
                            {
                                liquid.PumpLiquid(this.PumpSpeed * (float)gameTime.ElapsedGameTime.TotalMilliseconds / 1000f);
                                allfull = false;
                            }
                        }
                        if (allfull)
                    ...
                    else
                    {
                        ...
                        foreach (Liquid liquid in this.LinkedObjects)
                        {
                            if (!liquid.IsEmpty)
                            {
                                liquid.PumpLiquid(-this.PumpSpeed * 0.03f * (float)gameTime.ElapsedGameTime.TotalMilliseconds / 1000f);
                                allEmpty = false;
                            }
                        }
                        if (allEmpty)
