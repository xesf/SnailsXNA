...
namespace TwoBrainsGames.Snails.Stages.HUD
{
    public class StageHUD : IBrainComponent
    {
        ...
        public bool ControlButtonsVisible { get { return this._itemControls._visible; } }
        ...
        public void LoadContent()
        {
            ...
            //this._font = BrainGame.ResourceManager.Load<TextFont>(FontResources.MAIN_FONT_SMALL, ResourceManager.ResourceManagerCacheType.Static);
            this._toolsMenu.LoadContent();

            // Stage area depends on the tools width so this will have to be set here
            _stageArea = new BoundingSquare(new Vector2(0.0f), this._toolsMenu._ToolsArea.Left, SnailsGame.ScreenHeight);

          //  float diagonal = (float)Math.Sqrt((_stageArea.Width * _stageArea.Width) + (_stageArea.Height * _stageArea.Height));
            // We need stage Area so we initialize it here
            if (SnailsGame.GameSettings.MinimapVisible)
            {
        ...
        {
            // Only allow one incoming message at the time
            // A queue may be implemented, but for now this will do. Let's see in the future
            if (!this._incomingMessage.IsActive)
            {
