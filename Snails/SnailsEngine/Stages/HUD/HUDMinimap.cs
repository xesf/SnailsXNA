...
namespace TwoBrainsGames.Snails.Stages.HUD
{
    class HUDMinimap : HUDItem
    {
                ...
                for (int c = 0; c < boardColumns; c++)
                {
                        ...
                        tileCell.Tile.Sprite != null)
                    {
                        ...
                        if (tileCell.Tile.IsBreakable)
                            item.type = 1;
                        item.position = new Vector2((int)_position.X + c * tileSizeX, (int)_position.Y + r * tileSizeY);
        ...
        public override void Update(BrainEngine.BrainGameTime gameTime)
        {
            ...
            camX += (int)_position.X + 1;
            camY += (int)_position.Y + 1;
        }

        public override void Draw(SpriteBatch spriteBatch)
        {
            //minimap background
            SnailsGame.DrawRectangleFilled(spriteBatch, new Rectangle((int)_position.X, (int)_position.Y, MAP_WIDTH, MAP_HEIGHT), Color.Black);
            ...
            //minimap frame
            SnailsGame.DrawRectangleFrame(spriteBatch, new Rectangle((int)_position.X, (int)_position.Y, MAP_WIDTH, MAP_HEIGHT), Color.Blue, 1);
            // camera
            ...
            {
                int x = ConvertToMinimapPointX((int)snail.Position.X) + (int)_position.X;
                int y = ConvertToMinimapPointY((int)snail.Position.Y) + (int)_position.Y - tileSizeY;
