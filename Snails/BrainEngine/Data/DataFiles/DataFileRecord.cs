...
namespace TwoBrainsGames.BrainEngine.Data.DataFiles
{
    public class DataFileRecord
    {
        ...
        public void AddField(string name, object value, bool update)
        {
            ...
            {
                DataFileField field = this.GetFieldByName(name);
                if (field == null)
                {
                    this.AddField(new DataFileField(name, value));
                }
        ...
        public T GetFieldValue<T>(string fieldName)
        {
            ...
            {
                return (T)(object)(this.GetVector2Value(field));
            }

            // Size
            if (typeof(T) == typeof(TwoBrainsGames.BrainEngine.UI.Size))
            {
                Microsoft.Xna.Framework.Vector2 v = this.GetVector2Value(field);
                return (T)(object)(new TwoBrainsGames.BrainEngine.UI.Size(v.X, v.Y));
            }
        ...
        private float GetFloatValue(DataFileField field)
        {
            ...
            if (field.Value.GetType() == typeof(string))
            {
                return ParseFloat(field.Value.ToString());
            }
            if (field.Value.GetType() == typeof(int) ||
                field.Value.GetType() == typeof(bool))
            {
        ...
        {
            string str = floatString.Replace(",", CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator);
        ...
        {
            return Parsers.ParseColor(colorString);
        }

        /// <summary>
        /// 
        /// </summary>
        private Microsoft.Xna.Framework.Vector2 GetVector2Value(DataFileField field)
        {
            if (field.Value.GetType() == typeof(Microsoft.Xna.Framework.Vector2))
            {
                return (Microsoft.Xna.Framework.Vector2)field.Value;
            }
            if (field.Value.GetType() == typeof(string))
            {
                return this.ParseVector2(field.Value.ToString());
            }
        ...
        /// 
        /// </summary>
        private Microsoft.Xna.Framework.Vector2 ParseVector2(string vector2String)
        {
            string [] nibbles = vector2String.Split(',');
            float x = 0;
            if (nibbles.Length > 0)
            {
                x = this.ParseFloat(nibbles[0]);
            }

            float y = 0;
            if (nibbles.Length > 1)
            {
                y = this.ParseFloat(nibbles[1]);
            }
