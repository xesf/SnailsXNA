...
namespace TwoBrainsGames.BrainEngine.Advertising
{
	...
	{
		private double RETRY_TIME_MSECS = 30000; 
		...
		public override void OnUpdate(BrainGameTime gameTime)
		{
			...
			if (this._retryAdMob) {
				this._retryEllapsedTime += gameTime.ElapsedRealTime.TotalMilliseconds;
				if (this._retryEllapsedTime >= RETRY_TIME_MSECS) {
					this.RequestAdMob ();
				}
			}
		...
		public override void Show()
		{
			...
			this._adMobAdVisible = false;
			this._offLineAdAnim.Visible = true;
			this._retryAdMob = true;
			this._retryEllapsedTime = RETRY_TIME_MSECS - 3000;
			//this.RequestAdMob ();
