...
namespace TwoBrainsGames.BrainEngine.UI.Controls
{
    public class UIScrollablePanel : UIPanel
    {
        ...
        public bool AtEnd()
        {
            if (this.Orientation == PanelOrientation.Horizontal)
            {
                if (this.Width < this.Parent.Size.Width)
                {
                    return true;
                }
                return (Math.Abs(this.Position.X) == Math.Abs(this.Width - this.Parent.Size.Width));
            }

            if (this.Height < this.Parent.Size.Height)
            {
                return true;
            }
            return (Math.Abs(this.Position.Y) == Math.Abs(this.Height - this.Parent.Size.Height));
        }
        ...
        public override void BeginDraw()
        {
            ...
            BrainGame.GraphicsManager.GraphicsDevice.RasterizerState = rastState;
            BrainGame.GraphicsManager.GraphicsDevice.ScissorRectangle = this.ClipRect;
  
            this.SpriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, BrainGame.CurrentSampler, null, rastState);
