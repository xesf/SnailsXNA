...
namespace TwoBrainsGames.BrainEngine.UI.Screens
{
    public class ScreenNavigator
    {
		...
		bool _loadingGroupFirstPass; // Deixamos passar uma vez no update para mostrar o icon de loading, na segunda vez faz o loading - leituras sincronas
		string _groupLoadParam;
		int _currentScreenDataLoadingIdx;
...
#warning TODO Corrigir isto para o Windows 8
     ...
     //   bool _previousGameActiveState;
        ...
        public SpriteBatch SpriteBatch
        {
            ...
            set { this._spriteBatch = value; } // tests to override Viewport definitions
        }
        ...
        public void LoadContent()
        {
			...
			DataFileRecord record = BrainGame.ResourceManager.Load<DataFileRecord>(Path.Combine(BrainGame.Settings.NavigatorContentFolder, BrainGame.Settings.NavigatorContentId), ResourceManager.ResourceManagerCacheType.Static);
			this._screensData = ScreensData.FromDataFileRecord (record);
		...
		private void LoadGroup(string groupId)
		{
			...
			{
				if (this._currentScreenDataLoadingIdx < this._screensData._groupsData [groupId].ScreensData.Count) 
				{
					this.LoadScreenData (this._screensData._groupsData [groupId].ScreensData[_currentScreenDataLoadingIdx]);
					this._currentScreenDataLoadingIdx++;
				}
				else
				{
					this._isLoadingGroup = false;
		...
		/// 
		/// </summary>
		private void LoadScreenData(ScreensData.ScreenData screenData)
		{
		...
		private void LoadGroupData(string groupId)
		{
			...
			{
				this.LoadScreenData (screenData);
			}
			this._currentGroupId = groupId;
		}
        ...
        public void Update(BrainGameTime gameTime)
        {
			...
			{
				if (BrainGame.Settings.UseAsyncLoading) 
				{
				...
				{
					if (!this._loadingGroupFirstPass) {
						LoadGroup (this._groupLoadParam);
					}

					this._loadingGroupFirstPass = false;
				}
				return;
			}
        ...
        private void ProcessScreenAction(ScreenAction action, BrainGameTime gameTime)
        {
			...
			case ScreenActionType.GroupLoad:
				if ((string)action._param != this._currentGroupId)
				{
					...
					{
						this._isLoadingGroup = true;
						this._groupLoadParam = (string)action._param;
						this._loadingGroupFirstPass = true;
						this._currentScreenDataLoadingIdx = 0;
						this.ShowLoadingIcon ();
					}	 
					else 
					{
						LoadGroup ((string)action._param);
					}
				}
				break;
        ...
        public void Draw()
        {
            ...
            if (BrainGame.DisplayHDDAccessIcon && 
                ...
                BrainGame.HddAccessIcon.Visible)
            {
                this._spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, BrainGame.CurrentSampler, null, null, BrainGame.RenderEffect);
                BrainGame.HddAccessIcon.Draw(this._spriteBatch);
            ...
            {
                this._spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, BrainGame.CurrentSampler, null, null, BrainGame.RenderEffect);
                BrainGame.AchievementsManager.Draw(this._spriteBatch);
                this._spriteBatch.End();
            }
