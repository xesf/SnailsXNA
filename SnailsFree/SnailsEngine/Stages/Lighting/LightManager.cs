...
namespace TwoBrainsGames.Snails.Stages
{
    public class LightManager
    {
        ...
        public void Draw()
        {
            ...
            // Tint the lights
            this._spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, null, null, null, Stage.CurrentStage.Camera.StageRenderEffect); 
            foreach (LightSource source in this._lightSources)
            {
                if (source.IsOn)
                {
                    source.DrawTint(this._spriteBatch);
                }
            }
            ...
            this._spriteBatch.Draw(this._lightMapRenderTarget, Vector2.Zero, Color.White);
            this._spriteBatch.End();
        }
        ...
        public void CreateMap()
        {
            ...
            this._spriteBatch.Begin(SpriteSortMode.Immediate, this._blendState, null, null, null, Stage.CurrentStage.Camera.StageRenderEffect); 
            foreach(LightSource source in this._lightSources)
            {
                if (source.IsOn)
                {
                    source.Draw(this._spriteBatch);
                }
            }
            this._spriteBatch.End(); 
