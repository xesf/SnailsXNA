...
namespace TwoBrainsGames.Snails.Stages
{
	...
	public class InGameCamera : Camera2D
	{
		...
		public Rectangle CameraRect { get; set; } // Utilizado para fazer culling no draw dos tiles
		...
		public override void Update(BrainGameTime gameTime)
		{
			...
			this.CameraRect = new Rectangle (
				(int)(this.Position.X - ((this.Origin.X )/ this.Scale.X)),
				(int)(this.Position.Y - ((this.Origin.Y)/ this.Scale.Y)), 
				(int)((StageArea.Width)/ this.Scale.X), 
				(int)((StageArea.Height)/ this.Scale.Y));
