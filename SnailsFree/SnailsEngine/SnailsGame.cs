...
namespace TwoBrainsGames.Snails
{
    ...
    public class SnailsGame : BrainGame, IDisposable
    {
        ...
        const string MICROSOFT_PUB_BLOCKID = "140372"; 
        ...
        {
            _instance = this;
            BrainGame.GameName = "Snails Free";
            BrainGame.GameFolderName = "SnailsFree";
            BrainGame.BetaName = "Snails Free Beta";
        }
		...
		protected override void OnInitialize()
        {
			...
			if (BrainGame.Settings.UseAds) {
				#if IOS
				Ad.OffLineAdSprite =  BrainGame.ResourceManager.GetSprite ("spriteset/ads/offline-ad", ResourceManager.RES_MANAGER_ID_STATIC);
				#endif
			}
        }
        ...
        public override void SetupRenderViewport()
        {
            ...
            // Calcular AR do écran (janela se não for fullscreen, ou do écran todo em fullscreen)
          ...
          //  this._graphicsManager.IsFullScreen = false;
           // this._graphicsManager.ApplyChanges();
            // Agora, uma de duas:
            //   1) Ou ajustamos a altura do jogo à janela/ecran e ficamos com barras laterais pretas
            //   2) Ou ajustamos a largura do jogo à janela/ecran e ficamos com barras em cima e baixo
            // Isto pode-se ver pela comparação dos aspect ratios
            ...
            if (aspectRatioGame > aspectRatioScreen)
            {
                ...
                // Centramos no ecran no y (no x não precisa porque enche o ecran todo
            ...
            else // Igual? Facil, é só meter o viewport igual a janela/ecran
            {
        ...
        public override TwoBrainsGames.BrainEngine.UI.Screens.Screen CreateScreen(ScreenNavigator navigator, string screenId)
        {
            ...
            switch (type)
            {
                ...
                case ScreenType.PlayerStats:
                    ...
                    break;
                case ScreenType.HintWarning:
                    screen = (Screen)new HintWarningScreen(navigator);
                    break;
        ...
        private bool EULAAgreement()
        {
            ...
            SnailsGame.ProfilesManager.CurrentProfile.EULAAcceptedInVersion = SnailsGame.GameVersion;
            return true;
#else
        ...
        /// 
        /// </summary>
        public static void RedirectToPaidSnailsOnStore()
        {
            #if APP_STORE_SUPPORT
            ...
            try
            {
                ...
                marketplaceDetailTask.ContentIdentifier = "4413f8fc-bc8c-404b-a39c-82344f237584";
