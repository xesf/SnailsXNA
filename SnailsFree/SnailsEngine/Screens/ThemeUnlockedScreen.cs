...
namespace TwoBrainsGames.Snails.Screens
{
	...
	{
		#region Consts
		private const float LINE_SPACING = 25f;
		#endregion
		...
		public override void OnLoad()
		{
			...
			this._board.ImagePosition = this.PixelsToScreenUnits (new Vector2 (0, 50));
			this._board.Size = new BrainEngine.UI.Size(this._board.Width, this.PixelsToScreenUnitsY(470));
			...
			this._title.Position = this.PixelsToScreenUnits (new Vector2(0, -35));
			...
			this._imgTheme.Position = this.PixelsToScreenUnits (new Vector2(0, 65));
			...
			this._btnStageSel.Position = this.PixelsToScreenUnits(new Vector2(70, 0));
			...
			this._btnProceed.Position = this.PixelsToScreenUnits(new Vector2(255, 0));
			this._board.Controls.Add(this._btnProceed);

			// Stars
			this._stars = new UIStars(this, 20, 2600, 500, 1000);
			this._stars.Name = "_stars";
			this._stars.Size = new BrainEngine.UI.Size(this._board.Size.Width, this.PixelsToScreenUnitsY(70));
			this._stars.Position = this.PixelsToScreenUnits (new Vector2 (0, 300));
			...
			Vector2 pos = this.PixelsToScreenUnits(new Vector2(0, 275));
			for (int i = 0; i < helpText.Length; i++)
			{
				this._capHelp[i].Position = pos;
				pos += new Vector2(0f, this.PixelsToScreenUnitsY(LINE_SPACING));
			}
		}
