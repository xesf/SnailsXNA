...
namespace TwoBrainsGames.Snails.Screens
{
	...
	class StageCompletedScreen : SnailsScreen
	{
		...
		public override void OnLoad()
		{
			...
			this._leafsBoard.ParentAlignment = AlignModes.HorizontalyVertically;
			...
			this._btnAgain.Position = this.PixelsToScreenUnits(new Vector2(165, 0));
			...
			this._btnQuit.Position = this._btnAgain.Position - this.PixelsToScreenUnits(new Vector2(145, 0));
			...
			this._btnNext.Position = this._btnAgain.Position + this.PixelsToScreenUnits(new Vector2(145, 0));
			...
			this._medal.Position = this.PixelsToScreenUnits (new Vector2(10, 320));
			...
			this._capSnailsDelivered.Position = this.PixelsToScreenUnits(new Vector2(50, 110));
			this._capTimeTaken.Position = this._capSnailsDelivered.Position + this.PixelsToScreenUnits(new Vector2(0.0f, 30));
			this._capSnailsBonus.Position = this._capTimeTaken.Position + this.PixelsToScreenUnits(new Vector2(0.0f, 50));
			this._capTimeBonus.Position = this._capSnailsBonus.Position + this.PixelsToScreenUnits(new Vector2(0.0f, 30));
			this._capCoinsBonus.Position = this._capTimeBonus.Position + this.PixelsToScreenUnits(new Vector2(0.0f, 30));
			this._capTotalPoints.Position = this._capCoinsBonus.Position + this.PixelsToScreenUnits(new Vector2(0.0f, 50));

			this._capSnailsDelivered.Width = this.PixelsToScreenUnitsX(360);
			this._capTimeTaken.Width = this._capSnailsDelivered.Width;
			this._capSnailsBonus.Width = this._capSnailsDelivered.Width;
			this._capTimeBonus.Width = this._capSnailsDelivered.Width;
			this._capCoinsBonus.Width = this._capSnailsDelivered.Width;
			this._capTotalPoints.Width = this._capSnailsDelivered.Width;
		}
		...
		/// 
		/// </summary>
		protected void btnLeaderboards_OnAccept(IUIControl sender)
		{
			#if GAMER_SERVICES
			Guide.ShowLeaderboard(Stage.CurrentStage.LevelStage.ToStageKeyUnformatted());
			this.EnableInput();
			#endif
		}
			...
			if (SnailsGame.ProfilesManager.CurrentProfile != null)
			{
				...
				if (playerStats != null)
				{
					...
					if (Stage.CurrentStage.Stats.TotalScore >= playerStats.Highscore)
					{
						...
						#if GAMER_SERVICES
						if (SnailsGame.GameSettings.UseGamerServices &&
						Gamer.SignedInGamers.Count > 0 &&
						Gamer.SignedInGamers [0].IsSignedInToLive) 
						{
						Gamer.SignedInGamers[0].UpdateScore(Stage.CurrentStage.LevelStage.ToStageKeyUnformatted(), playerStats.Highscore);
						}
						#endif
					}
				...
				if (totalStagesUnlocked == Levels._instance.StageCount) // all stages unlocked
				{
