...
namespace TwoBrainsGames.Snails.Screens.CommonControls
{
    ...
    class UIThemeScrollablePanel : UISnailsScrollablePanel
    {
		...
		base(screenOwner, UIScrollablePanel.PanelOrientation.Horizontal, false, 13700f)
        {
			float offset = 4500;
			float posy = 300;
            ...
            // Garden
			this._themesControls[(int)ThemeType.ThemeA] = this.CreateThemeControl(ThemeType.ThemeA, new Vector2(0, posy));
            ...
            // Acient egypt
			this._themesControls[(int)ThemeType.ThemeB] = this.CreateThemeControl(ThemeType.ThemeB, new Vector2(offset, posy));
            // Graveyard
			this._themesControls[(int)ThemeType.ThemeC] = this.CreateThemeControl(ThemeType.ThemeC, new Vector2(offset * 2, posy));

			this.Size = new Size(9000, 4800); 
        ...
        private UITheme CreateThemeControl(ThemeType themeId, Vector2 position)
        {
			...
			theme.Margins.Top = this.PixelsToScreenUnitsY(35);
			theme.Position = position;
			theme.ParentAlignment = AlignModes.Vertically;
			...
			float f = 4500 * theme.SizeInPixels.Width / theme.Size.Width; 
			float scalex = f / (float)theme.SizeInPixels.Width;
			theme.Scale = new Vector2 (scalex, scalex);
